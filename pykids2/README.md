...
Stored in the browser's `localStorage` under key `pykids-progress-v1`, per
device (no login needed):
```json
{ "python-basics": { "completedLessons": ["lesson1", "lesson2"] } }
```
- First lesson in a track is always unlocked; each next lesson unlocks once
  the one before it is marked complete (see `isLessonUnlocked()` in
  `progress.js`).
- The lesson page's **"Mark Complete ✓"** button records progress and
  auto-advances to the next lesson.
- The home page shows a mini progress bar per track and a **"Reset my
  progress"** link.
- This is per-browser/device only. If you later add accounts, swap
  `loadProgress()`/`saveProgress()` in `progress.js` for API calls — nothing
  else in the framework needs to change.

## Badges / achievements

Defined entirely in `config/badges.config.json` — add a new badge without
touching any code:
```json
{ "id": "high-five", "title": "High Five", "icon": "🖐️",
  "description": "Complete 5 lessons total",
  "rule": { "type": "lesson-count", "count": 5 } }
```
**Rule types supported today:**
| type | meaning |
|---|---|
| `lesson-count` | earned once N lessons are completed, across any track(s) |
| `track-complete` | earned once every lesson in a specific track (`"track": "<id>"`) is done |

Add a new rule type by extending `badgeIsEarned()` in `assets/js/badges.js`.

- **`badges.html`** — gallery showing earned vs. locked badges.
- **Home page** — shows a "🏅 X / Y badges earned" link.
- **Lesson page** — when "Mark Complete" triggers a *new* badge, a toast
  pops up before advancing to the next lesson.

## Roadmap
- [x] Python Basics track (6 lessons) — config-driven
- [x] Reusable framework: master config + track JSON + lesson JSON
- [x] Progress tracking (localStorage) + lesson lock/unlock
- [x] Badges / achievements (config-driven, `badges.config.json`)
- [x] Advanced Python track (4 lessons — error handling, files, classes, capstone)
- [x] AI for Kids track (4 lessons — rules, classifiers, chatbots, capstone)
